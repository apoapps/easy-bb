...
describe('Next backend helpers', () => {
  it('does not ask for an external backend URL', async () => {
    const response = jsonError(502, 'BLACKBOARD_UNAVAILABLE', 'Could not connect to Blackboard.');
  ...
  it('validates login input on the Next backend', async () => {
    await expect(parseLoginBody(new Request('http://localhost/api/login', {
      method: 'POST',
      body: JSON.stringify({ school: 'school', username: 'user0001', password: 'secret' }),
    }))).resolves.toEqual({ school: 'school', username: 'user0001', password: 'secret' });
  });
});
